...
hdiutil create -size 200m "$TMP/$WORK_TEMPLATE" -type SPARSE -fs HFS+ -volname "KClient"
  ...
  hdiutil attach "$TMP/$WORK_TEMPLATE" -mountpoint "$TMP/$VOLUME_TEMPLATE"
    ...
    mkdir "$TMP/$VOLUME_TEMPLATE/KClient.app"
    ...
    cp -fpR "$BUNDLE_PATH/KClient.app/Contents" "$TMP/$VOLUME_TEMPLATE/KClient.app"
...
rm "$BUNDLE_PATH/KClient.dmg"
hdiutil convert "$TMP/$WORK_TEMPLATE" -format UDBZ -o "$BUNDLE_PATH/KClient.dmg"
