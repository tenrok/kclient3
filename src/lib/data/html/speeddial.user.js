// ==UserScript==
// @name     _kclient_speeddial
// @run-at   document-end
// @include  kclient:speeddial
// ==/UserScript==
...
function configureSpeedDial() {
	...
	if ($('#BgImgToggle').prop('checked') != true) {
		$('#ImgSelectorMenu').css({'color': 'rgba(0,0,0,.0)', 'text-shadow': 'none'});
		$('#BgImgSel').css({'color': 'rgba(0,0,0,.0)', 'text-shadow': 'none'});
		$('#BgImgSelSiz').css('visibility', 'hidden');
	}
	if ($('#SdSizeToggle').prop('checked') != true)
		$('#SdSizeStateColor').css('color', 'rgba(0,0,0,.0)');
	// ======================== SHOW DIALOG ======================== //
...
function onFetchTitleClick(checkbox) {
	$('#titleLine').css({'visibility': checkbox.checked ? 'hidden' : 'visible'});
}
...
function reloadAll() {
	if (confirm(scriptData.titleWarnRel)) $('div.entry').each(function(i) {onReloadClick($(this));});
}
...
function removeBox(id) {
	var box;
	if (confirm(scriptData.titleWarn)) box = document.getElementById('quickdial').getElementsByTagName('div')[id];
	if (box === undefined) return;
...
function alignPage() {
	...
	$('#quickdial div.entry').css({'width': dialWidth + 'px', 'height': dialHeight  + 'px'});
...
function bgImgToggle() {
	...
	if ($('#BgImgToggle').prop('checked') != true) {
		...
		$('#ImgSelectorMenu').css({'color': '#eaeaea', 'text-shadow': '1px 1px 2px #000,0 0 1em #000'});
		$('#BgImgSel').css({'color': '#eaeaea', 'text-shadow': '1px 1px 2px #000,0 0 1em #000'});
...
function enableCentering() {
	$('#quickdial div.entry').css({'float': 'none', 'display': 'inline-block'});
}

function disableCentering() {
	$('#quickdial div.entry').css({'float': scriptData.leftStr, 'display': 'block'});
}
