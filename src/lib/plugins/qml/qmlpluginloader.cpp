...
void QmlPluginLoader::initEngineAndComponent()
{
    ...
    m_engine->globalObject().setProperty(QSL("__kclient_i18n"), m_engine->newQObject(i18n));
    m_engine->evaluate(QSL("i18n = function (s) { return __kclient_i18n.i18n(s) };"));
    m_engine->evaluate(QSL("i18np = function (s1, s2) { return __kclient_i18n.i18np(s1, s2) }"));
#else
