...
PluginSpec Plugins::createSpec(const DesktopFile &metaData)
{
    ...
    spec.version = metaData.value(QSL("X-KClient-Version")).toString();
    spec.author = QSL("%1 <%2>").arg(metaData.value(QSL("X-KClient-Author")).toString(), metaData.value(QSL("X-KClient-Email")).toString());
    spec.hasSettings = metaData.value(QSL("X-KClient-Settings")).toBool();
...
void Plugins::loadPlugins()
{
    ...
    std::cout << "KClient: " << m_loadedPlugins.count() << " extensions loaded"  << std::endl;
}
...
void Plugins::loadAvailablePlugins()
{
    ...
    for (const QString &dir : dirs) {
        ...
        for (const QFileInfo &info : files) {
            if (info.baseName() == QL1S("PyKClient")) {
                continue;
            }
    ...
    // PythonPlugin
    if (m_pythonPlugin) {
        auto f = (QVector<Plugin>(*)()) m_pythonPlugin->resolve("pykclient_load_available_plugins");
        if (!f) {
            qWarning() << "Failed to resolve" << "pykclient_load_available_plugins";
        } else {
...
void Plugins::loadPythonSupport()
{
    const QStringList dirs = DataPaths::allPaths(DataPaths::Plugins);
    for (const QString &dir : dirs) {
        const auto files = QDir(dir).entryInfoList({QSL("PyKClient*")}, QDir::Files);
        for (const QFileInfo &info : files) {
            ...
            if (!m_pythonPlugin->load()) {
                ...
                std::cout << "KClient: Python plugin support initialized" << std::endl;
                return;
            }
        }
    }
...
Plugins::Plugin Plugins::loadPythonPlugin(const QString &name)
{
    ...
    auto f = (Plugin(*)(const QString &)) m_pythonPlugin->resolve("pykclient_load_plugin");
    if (!f) {
        qWarning() << "Failed to resolve" << "pykclient_load_plugin";
        return Plugin();
    }
...
void Plugins::initPythonPlugin(Plugin *plugin)
{
    ...
    auto f = (void(*)(Plugin *)) m_pythonPlugin->resolve("pykclient_init_plugin");
    if (!f) {
        qWarning() << "Failed to resolve" << "pykclient_init_plugin";
        return;
    }
