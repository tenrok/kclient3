...
QString Scripts::setupWebChannel()
{
    QString source =  QL1S("(function() {"
                           ...
                           "        event.initEvent('_kclient_external_created', true, true);"
                           "        window._kclient_external = true;"
                           "        document.dispatchEvent(event);"
                           "    }"
                           "}"
                           ""
                           "if (self !== top) {"
                           "    if (top._kclient_external)"
                           "        registerExternal(top.external);"
                           "    else"
                           "        top.document.addEventListener('_kclient_external_created', function() {"
                           "            registerExternal(top.external);"
                           "        });"
                           "    return;"
                           "}"
...
QString Scripts::setupWindowObject()
{
    QString source = QL1S("(function() {"
                          ...
                          "    window.location = 'kclient:AddSearchProvider?url=' + url;"
                          "};"
           ...
           source += QL1S("window.print = function() {"
                          "    window.location = 'kclient:PrintPage';"
                          "};");
#endif
           source += QL1S("})()");
