...
void ProfileManager::initConfigDir()
{
    ...
    std::cout << "KClient: Creating new profile directory" << std::endl;
...
void ProfileManager::updateProfile(const QString &current, const QString &profile)
{
    ...
    if (prof < Updater::Version(QStringLiteral("1.9.0"))) {
        std::cout << "KClient: Using profile from QupZilla " << qPrintable(profile) << " is not supported!" << std::endl;
        return;
    }
...
void ProfileManager::copyDataToProfile()
{
    ...
    if (browseData.exists()) {
        ...
        QMessageBox::warning(0, QStringLiteral("KClient: Incompatible profile version"), text);
    }
}
...
void ProfileManager::migrateFromQupZilla()
{
    ...
    std::cout << "KClient: Migrating config from QupZilla..." << std::endl;
