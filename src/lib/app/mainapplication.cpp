...
#endif
{
    ...
    setApplicationName(QStringLiteral("kclient"));
    setOrganizationDomain(QStringLiteral("org.kde"));
    setWindowIcon(QIcon::fromTheme(QSL("kclient"), QIcon(QSL(":icons/kclient.svg"))));
    setDesktopFileName(QSL("org.kde.kclient"));
    ...
    if (!isPortable()) {
        QSettings kclientConf(QSL("%1/kclient.conf").arg(applicationDirPath()), QSettings::IniFormat);
        m_isPortable = kclientConf.value(QSL("Config/Portable")).toBool();
    }

    if (isPortable()) {
        std::cout << "KClient: Running in Portable Mode." << std::endl;
        DataPaths::setPortableVersion();
    }

    // Don't start single application in private browsing
    if (!isPrivate()) {
        QString appId = QStringLiteral("org.kde.KClient");
...
void MainApplication::setupUserScripts()
{
    ...
    script.setName(QSL("_kclient_webchannel"));
    ...
    // kclient:restore
    QWebEngineScript kclientRestore;
    kclientRestore.setWorldId(WebPage::SafeJsWorld);
    kclientRestore.setSourceCode(QzTools::readAllFileContents(QSL(":html/restore.user.js")));
    m_webProfile->scripts()->insert(kclientRestore);

    // kclient:speeddial
    QWebEngineScript kclientSpeedDial;
    kclientSpeedDial.setWorldId(WebPage::SafeJsWorld);
    kclientSpeedDial.setSourceCode(Scripts::setupSpeedDial());
    m_webProfile->scripts()->insert(kclientSpeedDial);

    // document.window object addons
    QWebEngineScript documentWindowAddons;
    documentWindowAddons.setName(QSL("_kclient_window_object"));
...
void MainApplication::setUserStyleSheet(const QString &filePath)
{
    ...
    const QString name = QStringLiteral("_kclient_userstylesheet");
...
{
    qputenv("PULSE_PROP_OVERRIDE_application.name", "KClient");
    qputenv("PULSE_PROP_OVERRIDE_application.icon_name", "kclient");
    qputenv("PULSE_PROP_OVERRIDE_media.icon_name", "kclient");
}
...
RegisterQAppAssociation* MainApplication::associationManager()
{
    if (!m_registerQAppAssociation) {
        QString desc = tr("KClient is a new and very fast Qt web browser. KClient is licensed under GPL version 3 or (at your option) any later version. It is based on QtWebEngine and Qt Framework.");
        QString fileIconPath = QApplication::applicationFilePath() + ",1";
        QString appIconPath = QApplication::applicationFilePath() + ",0";
        m_registerQAppAssociation = new RegisterQAppAssociation("KClient", QApplication::applicationFilePath(), appIconPath, desc, this);
        m_registerQAppAssociation->addCapability(".html", "KClientHTML", "KClient HTML Document", fileIconPath, RegisterQAppAssociation::FileAssociation);
        m_registerQAppAssociation->addCapability(".htm", "KClientHTML", "KClient HTML Document", fileIconPath, RegisterQAppAssociation::FileAssociation);
        m_registerQAppAssociation->addCapability("http", "KClientURL", "KClient URL", appIconPath, RegisterQAppAssociation::UrlAssociation);
        m_registerQAppAssociation->addCapability("https", "KClientURL", "KClient URL", appIconPath, RegisterQAppAssociation::UrlAssociation);
        m_registerQAppAssociation->addCapability("ftp", "KClientURL", "KClient URL", appIconPath, RegisterQAppAssociation::UrlAssociation);
    }
