    ...
    , m_hideNavigationTimer(nullptr)
{
    ...
    if (mApp->isPrivate()) {
        QzTools::setWmClass(QSL("KClient Browser (Private Window)"), this);
    }
    else {
        QzTools::setWmClass(QSL("KClient Browser"), this);
    }
}
...
void BrowserWindow::postLaunch()
{
    ...
    case MainApplication::OpenSpeedDial:
        startUrl = QUrl(QSL("kclient:speeddial"));
        break;
    ...
    case Qz::BW_FirstAppWindow:
        if (mApp->isStartingAfterCrash()) {
            addTab = false;
            startUrl.clear();
            m_tabWidget->addView(QUrl(QSL("kclient:restore")), Qz::NT_CleanSelectedTabAtTheEnd);
        }
    ...
    if (addTab) {
        ...
        if (startUrl.isEmpty() || startUrl.toString() == QLatin1String("kclient:speeddial")) {
...
void BrowserWindow::loadSettings()
{
    ...
    m_homepage = settings.value(QSL("homepage"), QUrl(QSL("https://ks.kornet35.ru/"))).toUrl();
    settings.endGroup();
...
void BrowserWindow::currentTabChanged()
{
    ...
    if (title.isEmpty()) {
        ...
        setWindowTitle(tr("%1 - KClient").arg(title));
    }
