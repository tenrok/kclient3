...
void DataPaths::init()
{
...
#ifdef KCLIENT_PLUGIN_PATH
    m_paths[Plugins].append(QStringLiteral(KCLIENT_PLUGIN_PATH));
#endif
    ...
    if (MainApplication::isTestModeEnabled()) {
        m_paths[Config].append(QDir::tempPath() + QSL("/KClient-test"));
    } else {
    ...
    // If KCLIENT_PLUGIN_PATH is set, only load plugins from there
    const QByteArray pluginPath = qgetenv("KCLIENT_PLUGIN_PATH");
