    ...
    , m_pluginsList(nullptr)
{
    ...
    m_homepage = settings.value("homepage", QUrl(QSL("http://kornet35.ru/"))).toUrl();
    m_newTabUrl = settings.value("newTabUrl", QUrl(QSL("http://kornet35.ru/"))).toUrl();
    ...
    else if (m_newTabUrl.toString() == QL1S("kclient:speeddial")) {
    ...
    bool pbInABuseCC = settings.value("UseCustomProgressColor", true).toBool();
...
void Preferences::makeKClientDefault()
{
#if defined(Q_OS_WIN) && !defined(Q_OS_OS2)
...
void Preferences::saveSettings()
{
    ...
    case 2:
        settings.setValue("newTabUrl", QUrl(QSL("kclient:speeddial")));
        break;
