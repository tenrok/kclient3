...
struct KCLIENT_EXPORT PasswordEntry {
    QVariant id;
    QString host;
    QString username;
    QString password;
    ...
    friend KCLIENT_EXPORT QDataStream &operator<<(QDataStream &stream, const PasswordEntry &entry);
    friend KCLIENT_EXPORT QDataStream &operator>>(QDataStream &stream, PasswordEntry &entry);
};

class KCLIENT_EXPORT PasswordManager : public QObject
{
    Q_OBJECT
public:
