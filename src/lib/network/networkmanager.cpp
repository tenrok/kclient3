...
#include "schemehandlers/kclientschemehandler.h"
    ...
    : QNetworkAccessManager(parent)
{
    ...
    mApp->webProfile()->installUrlSchemeHandler(QByteArrayLiteral("kclient"), new KClientSchemeHandler());
    mApp->webProfile()->installUrlSchemeHandler(QByteArrayLiteral("extension"), m_extensionScheme);
    WebPage::addSupportedScheme(QSL("kclient"));
...
bool NetworkManager::certificateError(const QWebEngineCertificateError &error, QWidget *parent)
{
    ...
    QString text1 = tr("Страница, к которой вы пытаетесь получить доступ, содержит следующие ошибки в сертификате SSL:");
    QString text2 = tr("Хотите сделать исключение для этого сертификата?");
...
void NetworkManager::authentication(const QUrl &url, QAuthenticator *auth, QWidget *parent)
{
    ...
    save->setText(tr("Сохранить имя пользователя и пароль для этого сайта"));
    ...
    label->setText(tr("Имя пользователя и пароль запрашиваются %1. "
                      "Сайт сообщил: \"%2\"").arg(url.host(), auth->realm().toHtmlEscaped()));
...
void NetworkManager::proxyAuthentication(const QString &proxyHost, QAuthenticator *auth, QWidget *parent)
{
    ...
    QDialog* dialog = new QDialog(parent);
    dialog->setWindowTitle(tr("Требуется авторизация прокси"));
    ...
    label->setText(tr("Имя пользователя и пароль запрашиваются прокси %1. ").arg(proxyHost));
    formLa->addRow(label);
    formLa->addRow(userLab, user);
    formLa->addRow(passLab, pass);
    formLa->addWidget(box);
...
{
#if QTWEBENGINEWIDGETS_VERSION >= QT_VERSION_CHECK(5, 12, 0)
    QWebEngineUrlScheme kclientScheme("kclient");
    kclientScheme.setFlags(QWebEngineUrlScheme::SecureScheme | QWebEngineUrlScheme::ContentSecurityPolicyIgnored);
    kclientScheme.setSyntax(QWebEngineUrlScheme::Syntax::Path);
    QWebEngineUrlScheme::registerScheme(kclientScheme);
