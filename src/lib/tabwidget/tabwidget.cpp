    ...
    , m_tabWidget(tabWidget)
{
    ...
    setAcceptDrops(true);
    setToolTip(TabWidget::tr("Новая вкладка"));
}
    ...
    , m_closedTabsManager(new ClosedTabsManager)
{
    ...
    m_buttonClosedTabs->setToolTip(tr("Закрытые вкладки"));
    ...
    m_buttonListTabs->setToolTip(tr("Список вкладок"));
...
void TabWidget::loadSettings()
{
    ...
    m_newEmptyTabAfterActive = settings.value("newEmptyTabAfterActive", true).toBool();
    settings.endGroup();

    settings.beginGroup("Web-URL-Settings");
    m_urlOnNewTab = settings.value("newTabUrl", "kclient:speeddial").toUrl();
    settings.endGroup();
...
void TabWidget::aboutToShowClosedTabsMenu()
{
    ...
    if (m_menuClosedTabs->isEmpty()) {
        ...
        m_menuClosedTabs->addAction(tr("Восстановить все закрытые вкладки"), this, &TabWidget::restoreAllClosedTabs);
        m_menuClosedTabs->addAction(QIcon::fromTheme(QSL("edit-clear")), tr("Очистить список"), this, &TabWidget::clearClosedTabsList);
    }
}
