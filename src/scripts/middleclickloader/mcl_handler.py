...
class MCL_Handler(QtCore.QObject):
    ...
    def mousePress(self, view, event):
        ...
        if event.buttons() == QtCore.Qt.MiddleButton:
            ...
            if selectionClipboard:
                ...
                if not isValid:
                    searchManager = KClient.MainApplication.instance().searchEnginesManager()
    ...
    def loadUrl(self, view, url):
        if self.loadMode == MCL_LoadMode.NEW_TAB:
            view.openUrlInNewTab(url, KClient.Qz.NT_NotSelectedTab)
        elif self.loadMode == MCL_LoadMode.CURRENT_TAB:
            view.load(url)
        elif self.loadMode == MCL_LoadMode.NEW_WINDOW:
            KClient.MainApplication.instance().createWindow(KClient.Qz.BW_NewWindow, url)
        else:
            return False
        return True
