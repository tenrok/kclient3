# ============================================================
# HelloPython plugin for KClient
...
class HelloPlugin(KClient.PluginInterface, QtCore.QObject):
    buttons = {}

    def init(self, state, settingsPath):
        print("{} {}".format(state, settingsPath))

        plugins = KClient.MainApplication.instance().plugins()
        plugins.registerAppEventHandler(KClient.PluginProxy.MousePressHandler, self)
        ...
        KClient.SideBarManager.addSidebar("hellopython-sidebar", self.sidebar)

        self.schemeHandler = HelloSchemeHandler()
        KClient.MainApplication.instance().networkManager().registerExtensionSchemeHandler("hello", self.schemeHandler)

        if state == KClient.PluginInterface.LateInitState:
            for window in KClient.MainApplication.instance().windows():
                self.mainWindowCreated(window)

    def unload(self):
        print("unload")

        KClient.SideBarManager.removeSidebar(self.sidebar)
    ...
    def showSettings(self, parent):
        ...
        self.settings.setWindowIcon(QtGui.QIcon(":icons/kclient.svg"))
...
KClient.registerPlugin(HelloPlugin())

class HelloSchemeHandler(KClient.ExtensionSchemeHandler):
