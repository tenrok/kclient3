...
void TestPlugin::init(InitState state, const QString &settingsPath)
{
    ...
    // so it is recommended not to call any KClient function here
...
bool TestPlugin::testPlugin()
{
    ...
    return (Qz::VERSION == QLatin1String(KCLIENT_VERSION));
}
...
void TestPlugin::showSettings(QWidget* parent)
{
    ...
    if (!m_settings) {
        ...
        m_settings.data()->setWindowIcon(QIcon(":icons/kclient.svg"));
...
bool TestPlugin::mousePress(Qz::ObjectName type, QObject* obj, QMouseEvent* event)
{
    ...
    // Returning true may affect behaviour of KClient, so make sure you know what
    // you are doing!
    return false;
}
