...
{
    if (argc != 2) {
        std::cerr << "pykclienttestrunner [test.py]" << std::endl;
        return 1;
    }

    QzTools::removeRecursively(QDir::tempPath() + QSL("/KClient-test"));
    ...
    QLibrary library(QFINDTESTDATA("plugins/PyKClient." + suffix));
    ...
    auto run_script = (bool(*)(const QByteArray&)) library.resolve("pykclient_run_script");
    if (!run_script) {
        return 3;
    }
