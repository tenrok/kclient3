#!/bin/sh
###############################################################################
# Compile KClient source and pack it as Appimage.
###############################################################################
set -e
...
${BLD1}--sourcedir=${BLD0}
          ...
          Assuming this script is located in ${ITL1}kclient/linux${ITL0},
          otherwise you must specify the path to
          KClient source directory.

               ${UDR1}example:--sourcedir="/home/build/kclient"${UDR0}
...
${BLD1}--update-source | -U${BLD0}
          ...
          Fetches the information from KClient online git repository
...
-DKCLIENT_PLUGIN_PATH=""                     \
-DKDE_INSTALL_LIBDIR:PATH="${LIBDIRPATH}" ..

printf "Compiling KClient with the following settings:\n"
printConf
...
pushd bundle_build_dir/lib/plugins/kclient
...
CRYPTONEEDED="$(ldd 'bin/kclient'| grep libcrypto | sed 's/.*=>//;s/(.*//')"
...
cp ../linux/applications/org.kde.kclient.desktop bundle_build_dir
cp ../linux/pixmaps/kclient.png bundle_build_dir
ln -s kclient.png bundle_build_dir/.DirIcon

pushd bundle_build_dir
mv bin/kclient ./ && rm -fr bin
chrpath --replace '$ORIGIN' lib/libKClientPrivate.so.3.*
chrpath --replace '$ORIGIN/lib' kclient
...
KCLIENT_DIR="\$(dirname "\$(readlink -f "\$0")")"

XDG_DATA_DIRS="\${KCLIENT_DIR}/share:\${XDG_DATA_DIRS}"
KCLIENT_PLUGIN_PATH="\${KCLIENT_DIR}/lib/plugins/kclient"
export XDG_DATA_DIRS KCLIENT_PLUGIN_PATH
...
mksquashfs bundle_build_dir kclient.squashfs -root-owned -noappend

cat "${RUNTIME_BINARY}" >bin/KClient.AppImage
cat kclient.squashfs >>bin/KClient.AppImage
chmod a+x bin/KClient.AppImage
}
...
printf "Please install ${UDR1}$0${UDR0} in „${BLD1}scripts${BLD0}“ ${ITL1}(a sub folder in KClient source directory)${ITL0},
...
if [[ $? == 0 ]] && [[ -x bin/KClient.AppImage ]]; then
 printf "\\033c"
 printf "Done!\nThe compiled files are in "${PWD}"/bin\n"
 if [ ! -z "$OUT_DIR" ]; then
     cp bin/KClient.AppImage "$OUT_DIR"
 fi
fi
