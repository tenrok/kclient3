...
void PasswordBackendTest::storeTest()
{
    ...
    PasswordEntry entry;
    entry.host = QSL("org.kclient.google.com");
    ...
    PasswordEntry stored = m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).constFirst();
    QVERIFY(compareEntries(stored, entry) == true);

    reloadBackend();

    // Check entry retrieved from backend engine
    QVERIFY(!m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).isEmpty());
    stored = m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).constFirst();
    QVERIFY(compareEntries(stored, entry) == true);


    /* UTF-8 password entry */
    PasswordEntry entry2;
    entry2.host = QSL("org.kclient.kclient.com");
    ...
    // Check entry that may be stored in cache
    PasswordEntry stored2 = m_backend->getEntries(QUrl(QSL("org.kclient.kclient.com"))).constFirst();
    QVERIFY(compareEntries(stored2, entry2) == true);

    reloadBackend();

    // Check entry retrieved from backend engine
    stored2 = m_backend->getEntries(QUrl(QSL("org.kclient.kclient.com"))).constFirst();
    QVERIFY(compareEntries(stored2, entry2) == true);

    /* Cleanup */
    // Local cleanup
    m_backend->removeEntry(stored);
    QCOMPARE(m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).count(), 0);

    m_backend->removeEntry(stored2);
    QCOMPARE(m_backend->getEntries(QUrl(QSL("org.kclient.kclient.com"))).count(), 0);
...
void PasswordBackendTest::removeAllTest()
{
    ...
    QCOMPARE(m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).count(), 7);
    reloadBackend();
    QCOMPARE(m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).count(), 7);
...
void PasswordBackendTest::updateLastUsedTest()
{
    ...
    QVERIFY(!m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).isEmpty());
    QVERIFY(compareEntries(entry, m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).constFirst()));
    reloadBackend();
    QVERIFY(!m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).isEmpty());
    QVERIFY(compareEntries(entry, m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).constFirst()));

    m_backend->removeEntry(m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).constFirst());
    m_backend->removeEntry(m_backend->getEntries(QUrl(QSL("org.kclient.google.com"))).constFirst());
